...
important gain.
	...
	repeats it at the same regularisation. Both conditions, a non finite direction
	and a step below \texttt{degenerate\_step}, trigger the same response as a
	failed factorisation, with one addition: they also ratchet a floor under the
	regularisation that the schedule is not allowed to undo. Without the ratchet a
...
\end{remark}
	...
	constraint separately rather than for the problem as a whole. It is shorter
	than \texttt{bcl}.
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			$\kappa$ & \texttt{mu\_cut} & the hardest push allowed\\
			$\beta$ & \texttt{beta} & how fast the bar tightens\\
			$\mu_{\min}$ & \texttt{mu\_min} & the floor under every penalty\\
			$\bar w_0$ & \texttt{eps\_outer\_init} & the bar every row starts at\\
			$\bar w_{\min}$ & \texttt{smallest\_tolerance} & the floor under every
			bar\\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			\Require the violation per row, the reference value, whether the inner
			loop converged
			\For{every row $i$}
			...
			\EndFor
			\If{the inner loop converged}
			\State move all four centres onto the current iterate
			\State $\rho\gets\max(\rho_{\min},\ \varrho\,\rho)$ if $\rho$ is
			scheduled,\quad
			$\rho_p\gets\max(\rho_{p,\min},\ \varrho_p\,\rho_p)$
			\Comment{fixed factors}
			\EndIf
			\State $\mu\gets\max(\mu_{\min},\ \mu\odot f)$,\quad
			$\mu_s\gets\max(\mu_{\min},\ \mu_s\odot f)$
			\For{every row $i$}
			\State $\bar w_i\gets\max\bigl(\bar w_{\min},\
			\bar w_i\,\mu_i^{\beta}\bigr)$
			\Comment{the bar only ever tightens}
			\EndFor
			\State $r\gets$ the relative decrease of the reference since last time
			\State reduce $\rho$, if scheduled, and $\rho_p$ by $(1-r)$
			\Comment{progress buys a smaller proximal term}
		\end{algorithmic}
	\end{algorithm}
...
.
	...
	\begin{table}[H]
		...
		\begin{tabular}{lrrrrr}
			...
			\midrule
			random\_qp (90)          & \textbf{72}  & \textbf{72} & 70  & 67  & 68\\
			maros (138)              & \textbf{130} & 78  & 108 & 121 & 80\\
			netlib (109)             & \textbf{86}  & 38  & 67  & \textbf{86} & 31\\
			skfolio\_real\_qp (96)   & \textbf{96}  & 91  & 95  & 94  & 94\\
			skfolio\_random\_qp (60) & \textbf{60}  & 42  & 49  & 55  & 54\\
			seq\_rebalance (120)     & \textbf{120} & 117 & 118 & 115 & 115\\
			\midrule
			all (613)                & \textbf{564} & 438 & 507 & 538 & 442\\
			\bottomrule
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lrrrrr}
			...
			set & gqp interior & gqp semismooth & Clarabel & PIQP & ProxQP\\
			\midrule
			random\_qp (90)          & \textbf{0.144} & 0.226 & 0.176 & 0.203 & 0.339\\
			maros (138)              & \textbf{0.046} & 0.536 & 0.138 & 0.054 & 0.906\\
			netlib (109)             & 0.175 & 3.973 & 0.550 & \textbf{0.141} & 5.193\\
			skfolio\_real\_qp (96)   & \textbf{0.013} & 0.062 & 0.018 & 0.017 & 0.082\\
			skfolio\_random\_qp (60) & \textbf{0.041} & 0.395 & 0.115 & 0.084 & 0.141\\
			seq\_rebalance (120)     & \textbf{0.003} & 0.011 & 0.005 & 0.007 & 0.017\\
			\midrule
			all (613)                & \textbf{0.034} & 0.223 & 0.070 & 0.047 & 0.283\\
			\bottomrule
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lrrr}
			...
			\midrule
			random\_qp (90)               & \textbf{72}  & \textbf{72} & 70\\
			random\_socp (90)             & \textbf{71}  & 64  & 69\\
			random\_psd (60)              & 54  & \textbf{56} & 45\\
			random\_exp (60)              & \textbf{45}  & 36  & 44\\
			random\_pow (60)              & \textbf{57}  & 45  & 51\\
			random\_mixed (60)            & 34  & \textbf{36} & 27\\
			maros (138)                   & \textbf{130} & 78  & 108\\
			netlib (109)                  & \textbf{86}  & 38  & 67\\
			cblib (134)                   & \textbf{65}  & 31  & 19\\
			skfolio\_real\_qp (96)        & \textbf{96}  & 91  & 95\\
			skfolio\_real\_curved (192)   & \textbf{192} & 148 & 110\\
			skfolio\_random\_qp (60)      & \textbf{60}  & 42  & 49\\
			skfolio\_random\_curved (120) & \textbf{105} & 71  & 82\\
			seq\_rebalance (120)          & \textbf{120} & 117 & 118\\
			seq\_rebalance\_curved (120)  & \textbf{120} & 42  & 36\\
			\midrule
			all (1509)                    & \textbf{1307} & 967 & 990\\
			\bottomrule
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lrrr}
			...
			set & gqp interior & gqp semismooth & Clarabel\\
			\midrule
			random\_qp (90)               & \textbf{0.144} & 0.226 & 0.176\\
			random\_socp (90)             & 0.295 & 0.372 & \textbf{0.258}\\
			random\_psd (60)              & 0.071 & \textbf{0.044} & 0.242\\
			random\_exp (60)              & 0.542 & 1.195 & \textbf{0.277}\\
			random\_pow (60)              & 0.207 & 0.248 & \textbf{0.150}\\
			random\_mixed (60)            & 0.534 & \textbf{0.523} & 1.504\\
			maros (138)                   & \textbf{0.046} & 0.536 & 0.138\\
			netlib (109)                  & \textbf{0.175} & 3.973 & 0.550\\
			cblib (134)                   & \textbf{2.420} & 7.996 & 11.207\\
			skfolio\_real\_qp (96)        & \textbf{0.013} & 0.062 & 0.018\\
			skfolio\_real\_curved (192)   & \textbf{0.038} & 0.132 & 0.261\\
			skfolio\_random\_qp (60)      & \textbf{0.041} & 0.395 & 0.115\\
			skfolio\_random\_curved (120) & \textbf{0.142} & 1.186 & 0.274\\
			seq\_rebalance (120)          & \textbf{0.003} & 0.011 & 0.005\\
			seq\_rebalance\_curved (120)  & \textbf{0.129} & 0.927 & 1.431\\
			\midrule
			all (1509)                    & \textbf{0.099} & 0.395 & 0.258\\
			\bottomrule
		\end{tabular}
