...
Results run(const SparseMatrix& P, const Vector& q, const SparseMatrix& E,
            const Vector& b, const Cones& cones, Method method) {
  Settings settings;
  settings.method = method;
  return solve(P, q, E, b, cones, settings, nullptr);
}
...
void an_unconstrained_quadratic_reaches_its_minimum() {
  ...
  Vector b(0);
  const Cones cones{};
  const Results results = run(P, q, E, b, cones, Method::Interior);
...
void a_problem_with_no_rows_does_not_crash() {
  ...
  Vector b(0);
  const Cones cones{};
  const Results results = run(P, q, E, b, cones, Method::Semismooth);
...
void an_equality_pins_the_solution() {
  ...
  for (Method smoothing :
    ...
    const Results results = run(P, q, E, b, cones, smoothing);
    CHECK(results.status == Status::Solved);
...
void the_returned_slack_satisfies_the_splitting() {
  ...
  for (Index i = 0; i < 2; ++i)
    CHECK_CLOSE(image(i) + results.s(i), b(i), 1e-9);
}
