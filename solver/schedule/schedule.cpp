...
struct Gbcl final : Schedule {
  ...
  void update(Iterate& iterate, const Vector& violation, Scalar reference,
              Scalar, Scalar, bool inner_converged, std::size_t) override {
  ...
  const Vector& tolerance() const override { return accepted; }
  Scalar inner_tolerance() const override {
    return accepted.size() ? accepted.minCoeff() : settings.smallest_tolerance;
  }
...
struct GbclExp final : Schedule {
  explicit GbclExp(const ScheduleSettings& given) : settings(given) {}

  void setup(const Cones& given_cones, const Iterate& iterate, Scalar) override {
    cones = &given_cones;
    offsets = block_offsets(given_cones);
    factor.setOnes(iterate.s.size());
    largest_penalty = iterate.cone_penalty.size() ? iterate.cone_penalty.maxCoeff()
                                                  : settings.initial_penalty;
  ...
  void update(Iterate& iterate, const Vector& violation, Scalar,
    ...
    if (!inner_converged) {
      penalty_floor =
          std::max(penalty_floor,
                   std::min(largest_penalty / theta, settings.initial_penalty));
      for (Index row = 0; row < rows; ++row)
        factor(row) = violation(row) > accepted(row) ? 1.0 / theta : 1.0;
      flatten_on_curved(*cones, offsets, factor);
      iterate.cone_penalty =
          iterate.cone_penalty.cwiseProduct(factor).cwiseMin(
              settings.initial_penalty);
      iterate.equality_penalty =
          iterate.equality_penalty.cwiseProduct(factor).cwiseMin(
      ...
      for (Index row = 0; row < rows; ++row) {
        const Scalar ratio =
            violation(row) / std::max(accepted(row), Scalar(1e-300));
        factor(row) = std::min(
            Scalar(1), std::max(cut, std::pow(std::max(ratio, Scalar(1e-300)),
                                              -settings.violation_exponent)));
      }
      flatten_on_curved(*cones, offsets, factor);
      iterate.cone_penalty =
    ...
    if (primal_residual <= bar || taken >= settings.safe_guard) {
      iterate.x_centre = iterate.x;
      inner = std::max(inner * largest_penalty, settings.eps_absolute);
      bar = std::max(bar * std::pow(largest_penalty, settings.tighten_exponent),
                     settings.eps_absolute);
    } else {
      inner = std::max(std::min(inner, settings.newton_tolerance * largest_penalty),
                       settings.eps_absolute);
      bar = settings.initial_tolerance *
            std::pow(largest_penalty, settings.loosen_exponent);
    }
  ...
  const char* name() const override { return "gbcl_exp"; }
...
struct Interior final : Schedule {
  ...
  void update(Iterate& iterate, const Vector&, Scalar reference,
              Scalar primal_residual, Scalar dual_residual, bool, std::size_t) override {
    const Scalar rate =
...
struct InteriorExp final : Schedule {
  explicit InteriorExp(const ScheduleSettings& given) : settings(given) {}
  ...
  const char* name() const override { return "interior_exp"; }
...
std::unique_ptr<Schedule> make_schedule(ScheduleKind kind,
  ...
  switch (kind) {
    ...
    case ScheduleKind::GbclExp: return std::make_unique<GbclExp>(settings);
    case ScheduleKind::Interior: return std::make_unique<Interior>(settings);
    case ScheduleKind::InteriorExp: return std::make_unique<InteriorExp>(settings);
